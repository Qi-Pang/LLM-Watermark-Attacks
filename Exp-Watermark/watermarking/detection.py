...
def fast_permutation_test_query(tokens,vocab_size,n,k,seed,test_stat,null_results_list,null_id,args=None):
    generator = torch.Generator()
    generator.manual_seed(int(seed))
    ...
    test_result = test_stat(tokens=tokens,
                            ...
                            vocab_size=vocab_size)
    # FIXME: add dp noise
    if args is not None and (args.action == 'spoofing-defense' or args.action == 'dp-benchmark'):
        test_result = test_result + torch.normal(0, args.sigma, size=test_result.size())
    null_results = null_results_list[null_id]
    p_val = torch.searchsorted(null_results,test_result,right=True) / len(null_results)
    return p_val

def phi_query(tokens,n,k,generator,key_func,vocab_size,dist,null=False,normalize=False,xi=None,pi=None):
    if null:
