import argparse, tqdm, json, torch, pickle, os
...
def detect(sentence, wm_key, tokenizer, detector, args):
    ...
    z_score = detector.unidetect(tokens)
    return z_score
    ...
    if query_flag:
        for idx in range(len(tokens_list)):
            ...
            if prev is not None:
                ...
                new_list = token
            z_scores.append(detector.unidetect(new_list))
            total_query += 1
        # FIXME: add dp noise
        if args.action == 'spoofing-defense':
            token_len = new_list.shape[-1]
            sentivity = 1 / np.sqrt(detector.fraction * (1 - detector.fraction) * token_len)
...
def main(args):
    ...
    elif "llama-7b" in args.model_name_or_path.lower():
        results_folder = './llama-7b-results'
    else:
    ...
    if args.action == 'spoofing':
        ...
        os.makedirs(os.path.join(results_folder, 'api_spoofing'), exist_ok=True)

        spoofing_orig_scores_file_name = os.path.join(results_folder, 'api_spoofing', 'spoofing_orig_scores.pkl')
        spoofing_num_queries_file_name = os.path.join(results_folder, 'api_spoofing', 'spoofing_num_queries.pkl')
        spoofing_attack_scores_file_name = os.path.join(results_folder, 'api_spoofing', 'spoofing_attack_scores.pkl')
        spoofing_results_file_name = os.path.join(results_folder, 'api_spoofing', 'spoofing_results.pkl')
        spoofing_num_tokens_file_name = os.path.join(results_folder, 'api_spoofing', 'spoofing_num_tokens.pkl')
        ...
        for idx in tqdm.tqdm(range(args.T)):
            ...
            spoofing_orig_scores.append(detect(text, args.wm_key, tokenizer, detector, args))
            ...
            if len(watermarked_text) > 5:
                new_z = detect(watermarked_tokens[0], args.wm_key, tokenizer, detector, args)
        ...
        os.makedirs(os.path.join(results_folder, 'api_removal'), exist_ok=True)

        watermark_removal_num_queries_file_name = os.path.join(results_folder, 'api_removal', 'watermark_removal_num_queries.pkl')
        watermark_removal_attack_scores_file_name = os.path.join(results_folder, 'api_removal', 'watermark_removal_attack_scores.pkl')
        watermark_removal_results_file_name = os.path.join(results_folder, 'api_removal', 'watermark_removal_results.pkl')
        watermark_removal_num_tokens_file_name = os.path.join(results_folder, 'api_removal', 'watermark_removal_num_tokens.pkl')
        ...
        for idx in tqdm.tqdm(range(args.T)):
            ...
            if len(watermarked_text) > 5:
                detect_z_score = detect(watermarked_tokens[0], args.wm_key, tokenizer, detector, args)
        ...
        os.makedirs(os.path.join(results_folder, 'defense_api_spoofing'), exist_ok=True)

        spoofing_orig_scores_file_name = os.path.join(results_folder, 'defense_api_spoofing', 'spoofing_orig_scores.pkl')
        spoofing_num_queries_file_name = os.path.join(results_folder, 'defense_api_spoofing', 'spoofing_num_queries.pkl')
        spoofing_attack_scores_file_name = os.path.join(results_folder, 'defense_api_spoofing', 'spoofing_attack_scores.pkl')
        spoofing_results_file_name = os.path.join(results_folder, 'defense_api_spoofing', 'spoofing_results.pkl')
        spoofing_num_tokens_file_name = os.path.join(results_folder, 'defense_api_spoofing', 'spoofing_num_tokens.pkl')
        ...
        unwatermarked_file_name = os.path.join(results_folder, 'multiple_keys', 'detect_unwatermarked.pkl')
        watermarked_file_name = os.path.join(results_folder, 'multiple_keys', 'detect_watermarked.pkl')

        tokens_unwatermarked_file_name = os.path.join(results_folder, 'multiple_keys', 'unwatermarked.pkl')
        tokens_watermarked_file_name = os.path.join(results_folder, 'multiple_keys', 'watermarked.pkl')
        ...
        with open(tokens_unwatermarked_file_name, 'rb') as f:
            unwatermarked_tokens_list = pickle.load(f)
        
        with open(tokens_watermarked_file_name, 'rb') as f:
            watermarked_tokens_list = pickle.load(f)
        ...
        for idx in tqdm.tqdm(range(len(unwatermarked_detection_list))):
            z_score = unwatermarked_detection_list[idx]
            unwatermarked_score_list.append(z_score)

            # FIXME: add dp noise
            cur_tokens = unwatermarked_tokens_list[idx]
            token_len = tokenizer.encode(cur_tokens[0], return_tensors='pt', truncation=True, max_length=2048).shape[-1]
            sentivity = 1 / np.sqrt(detector.fraction * (1 - detector.fraction) * token_len)
            gaussian_scale = 2 * sentivity**2 * np.log(1.25 / args.delta) / (args.epsilon**2)
            # print(gaussian_scale)
        ...
        for idx in tqdm.tqdm(range(len(watermarked_detection_list))):
            z_score = watermarked_detection_list[idx]
            watermarked_score_list.append(z_score)

            # FIXME: add dp noise
            cur_tokens = watermarked_tokens_list[idx]
            token_len = tokenizer.encode(cur_tokens[0], return_tensors='pt', truncation=True, max_length=2048).shape[-1]
            sentivity = 1 / np.sqrt(detector.fraction * (1 - detector.fraction) * token_len)
            gaussian_scale = 2 * sentivity**2 * np.log(1.25 / args.delta) / (args.epsilon**2)
            dp_z_score = z_score + np.random.normal(0, gaussian_scale)
        ...
        accuracy = (np.sum(dp_unwatermarked_score_list < args.threshold) + np.sum(dp_watermarked_score_list >= args.threshold)) / (len(dp_unwatermarked_score_list) + len(dp_watermarked_score_list))

        unwatermarked_score_list = np.array(unwatermarked_score_list)
        watermarked_score_list = np.array(watermarked_score_list)

        orig_accuracy = (np.sum(unwatermarked_score_list < args.threshold) + np.sum(watermarked_score_list >= args.threshold)) / (len(unwatermarked_score_list) + len(watermarked_score_list))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', default='random', type=str,
